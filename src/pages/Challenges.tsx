...
export default function Challenges() {
  ...
  const fetchAll = async () => {
    ...
    // Compute end_date and ended status
    const now = new Date()
    ...
    // Filter joined to only active (not ended)
  ...
  return (
    <Layout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          <p style={{ color: '#555', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.3rem' }}>Compete</p>
          <h1 style={{ fontSize: '1.8rem', fontFamily: 'Space Grotesk', fontWeight: '700' }}>Challenges</h1>
          <p style={{ color: '#555', marginTop: '0.3rem', fontSize: '0.9rem' }}>Commit publicly. Build unstoppable momentum.</p>
        </div>
      ...
      <div style={styles.leavesBar}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <LogOut size={14} color={leavesLeft > 0 ? '#888' : '#E53E3E'} />
          <span style={{ color: leavesLeft > 0 ? '#888' : '#E53E3E', fontSize: '0.82rem' }}>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
          >
            <p style={{ color: '#555', fontSize: '0.78rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>New Challenge</p>
            <input
              style={styles.input}
            ...
            <button style={styles.saveBtn} onClick={createChallenge} disabled={creating}>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {joined.map((uc: any) => {
              ...
              return (
                <div key={uc.id} style={{ ...styles.card, borderColor: isComplete ? '#F5A62340' : '#1E1E1E' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                        {isComplete && <Award size={16} color="#F5A623" />}
                        <span style={{ fontWeight: '600', fontSize: '0.95rem' }}>{uc.challenges?.name}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <span style={{ color: '#555', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ color: '#F5A623', fontFamily: 'Space Grotesk', fontWeight: '700', fontSize: '1.1rem' }}>
                      ...
                      >
                        {checkedInList.length > 0 && (
                          <div style={{ marginBottom: '0.75rem' }}>
                            <p style={{ color: '#4CAF50', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem' }}>
                              Checked in today
                            </p>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                              {checkedInList.map((p: any, i: number) => (
                                <span key={i} style={{ ...styles.pill, background: '#0D2010', color: '#4CAF50', border: '1px solid #4CAF5030' }}>
                        ...
                        {missedToday.length > 0 && (
                          <div>
                            <p style={{ color: '#E53E3E', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.4rem' }}>
                              Missed today
                            </p>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                              {missedToday.map((p: any, i: number) => (
                                <span key={i} style={{ ...styles.pill, background: '#1A0000', color: '#E53E3E', border: '1px solid #E53E3E30' }}>
                  ...
                  {!isComplete && (
                    ...
                    >
                      <CheckCircle2 size={15} color={checkedInToday ? '#555' : '#0A0A0A'} />
                  ...
                  {!isComplete && (
                    <div style={{ marginTop: '0.75rem' }}>
                      {!isConfirmingLeave ? (
                        ...
                        >
                          <LogOut size={13} color={leavesLeft > 0 ? '#555' : '#333'} />
                          {leavesLeft > 0 ? `Leave challenge (${leavesLeft} left)` : 'No leaves remaining'}
                        </button>
                      ) : (
                        <div style={styles.confirmBox}>
                          <p style={{ color: '#E53E3E', fontSize: '0.85rem', margin: 0, marginBottom: '0.75rem' }}>
      ...
      {/* All challenges – only show active (not ended) */}
      <div style={styles.sectionLabel}>
        <Trophy size={13} color="#555" />
        All Challenges
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {challenges.filter(c => !c.ended).map(c => (
          <div key={c.id} style={styles.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontWeight: '600', marginBottom: '0.3rem', fontSize: '0.95rem' }}>{c.name}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <span style={{ color: '#555', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    <Clock size={12} color="#555" />
                    {c.days} days
                  ...
                  {c.profiles?.name && (
                    <span style={{ color: '#444', fontSize: '0.78rem' }}>by {c.profiles.name}</span>
                  )}
                  <span style={{ color: '#555', fontSize: '0.7rem', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
              ...
              {isJoined(c.id) ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#4CAF50', fontSize: '0.85rem' }}>
        ...
        {challenges.filter(c => !c.ended).length === 0 && (
          <p style={{ color: '#555', textAlign: 'center', padding: '1.5rem' }}>No active challenges right now. Create one!</p>
        )}
      </div>
    </Layout>
  )
