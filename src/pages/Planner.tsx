...
const TEMPLATES = [
  { title: 'Instagram Reel', platform: 'Instagram' },
  { title: 'X Thread', platform: 'X (Twitter)' },
  { title: 'TikTok Video', platform: 'TikTok' },
  { title: 'YouTube Short', platform: 'YouTube' },
  { title: 'LinkedIn Post', platform: 'LinkedIn' },
  { title: 'Threads Post', platform: 'Threads' },
  { title: 'Carousel Post', platform: 'Instagram' },
  { title: 'Podcast Episode', platform: '' },
  { title: 'Blog Article', platform: '' },
...
export default function Planner() {
  ...
  const addTask = async () => {
    ...
    await supabase.from('tasks').insert({
      user_id: user!.id, title, platform, date, completed: false, recurring
    })
    if (recurring) {
      for (let w = 1; w <= 7; w++) {
        ...
        await supabase.from('tasks').insert({
          user_id: user!.id, title, platform,
  ...
  const applyTemplate = (t: { title: string; platform: string }) => {
    setTitle(t.title)
        ...
        {showTemplates && (
          ...
          >
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {TEMPLATES.map(t => (
                <button key={t.title} style={styles.templateChip} onClick={() => applyTemplate(t)}>
                  {t.title}
                </button>
              ))}
            </div>
        ...
        {showForm && (
          ...
          >
            ...
            >
              <div style={{
                ...styles.checkbox,
                background: recurring ? '#F5A623' : 'transparent',
                borderColor: recurring ? '#F5A623' : '#333'
              }}>
            ...
            <div key={dateStr} style={{ ...styles.dayBlock, borderColor: isToday ? '#F5A62340' : '#1E1E1E' }}>
              <div style={styles.dayHeader}>
                ...
                <button
                  ...
                  onClick={() => { setDate(dateStr); setShowForm(true) }}
                >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: '0.5rem' }}>
                  {dayTasks.map(task => (
                    <div key={task.id} style={{ ...styles.taskRow, opacity: task.completed ? 0.45 : 1 }}>
                      <span style={{ cursor: 'pointer', display: 'flex' }} onClick={() => toggleTask(task)}>
                        {task.completed
...
const styles: Record<string, React.CSSProperties> = {
  ...
  progressTrack: {
    height: '6px', background: '#1E1E1E', borderRadius: '999px', overflow: 'hidden'
  },
  progressFill: {
  ...
  templateChip: {
    ...
    padding: '0.4rem 0.85rem', color: '#888', fontSize: '0.8rem',
    cursor: 'pointer', fontFamily: 'Inter'
  },
  form: {
    background: '#111111', border: '1px solid #1E1E1E', borderRadius: '14px',
    padding: '1.25rem', marginBottom: '1.25rem', display: 'flex',
